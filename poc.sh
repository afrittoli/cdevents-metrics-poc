...
KNATIVE_VERSION=${KNATIVE_VERSION:-1.5.0}
...
# Configure Tekton
retry_command "kubectl patch cm feature-flags -n tekton-pipelines -p {\"data\":{\"enable-api-fields\":\"alpha\"}}"
...
# DOMAIN=af-cdevents-dff43bc8701fcd5837d6de963718ad39-0000.eu-gb.containers.appdomain.cloud
...
metadata:
  ...
  annotations:
    ...
    nginx.ingress.kubernetes.io/auth-signin: "http://auth.${DOMAIN}/oauth2/sign_in?rd=http://\$host\$request_uri"
spec:
  rules:
  - host: tekton.${DOMAIN}
    http:
